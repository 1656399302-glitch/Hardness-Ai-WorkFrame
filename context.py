...
import config
from artifacts import write_latest_handoff
from runtime_state import append_event, increment_counter
...
def compact_messages(messages: list[dict], llm_call, role: str = "default") -> list[dict]:
    ...
    recent_start = _align_recent_boundary(non_system, len(non_system) - keep_count)
    old = non_system[:recent_start]
    ...
    append_event("context_compaction", f"Compacted {role} context", role=role)

    return system + [summary_msg] + recent


def _align_recent_boundary(messages: list[dict], boundary: int) -> int:
    """
    Move the recent-window boundary backward if it would otherwise start on a
    tool result whose matching assistant tool call was compacted away.
    """
    ...
    while idx >= 0:
        ...
        if msg.get("role") == "assistant":
            return idx if msg.get("tool_calls") else boundary
        idx -= 1
...
def create_checkpoint(messages: list[dict], llm_call) -> str:
    """
    Serialize current state into a structured JSON handoff artifact.
    ...
    checkpoint = llm_call([
        {"role": "system", "content": (
            ...
            "{\n"
            '  "completed_work": [strings],\n'
            '  "current_state": [strings],\n'
            '  "next_steps": [strings],\n'
            '  "key_decisions": [strings],\n'
            '  "known_issues": [strings],\n'
            '  "key_files": [strings],\n'
            '  "restore_instructions": [strings]\n'
            "}\n\n"
            "Be concrete: file paths, function names, commands, and failures."
        )},
    ...
    try:
        ...
        payload = {
            "completed_work": [],
            "current_state": ["Checkpoint JSON generation failed; see raw payload."],
            "next_steps": ["Read the current workspace files before continuing."],
            "key_decisions": [],
            "known_issues": [],
            "key_files": [],
            "restore_instructions": ["Use the current workspace files as source of truth."],
            "raw_checkpoint": checkpoint,
        }

    write_latest_handoff(payload, config.WORKSPACE)
    increment_counter("resets")
    append_event("context_reset", "Agent context reset via structured handoff")
    log.info("Checkpoint written to structured handoff artifact")
...
def restore_from_checkpoint(checkpoint: str, system_prompt: str) -> list[dict]:
    ...
    return [
        ...
        {"role": "user", "content": (
            ...
            "Here is the structured handoff from the previous session:\n\n"
            + checkpoint
            + git_context
