        ...
        with sync_playwright() as p:
            ...
            page.on(
                ...
                lambda request: request_failures.append(
                    f"{request.method} {request.url} — {request.failure.error_text if request.failure else 'unknown error'}"
                ),
            )
            ...
            if page_errors:
                report_lines.append(f"Page errors ({len(page_errors)}):")
                for err in page_errors[:10]:
                    report_lines.append(f"  - {err[:200]}")
            if request_failures:
                report_lines.append(f"Request failures ({len(request_failures)}):")
                for err in request_failures[:10]:
                    report_lines.append(f"  - {err[:200]}")
